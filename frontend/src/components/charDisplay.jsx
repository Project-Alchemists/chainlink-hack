...
const CharDisp = (props) => {
  ...
  try {
    full = Array(props.hearts).fill(
      <img src={fheart} alt="" className="list-heart"></img>
    );
    empty = Array(4 - props.hearts).fill(
      <img src={eheart} alt="" className="list-heart"></img>
    );
  } catch (error) {
    full = [];
    empty = Array(4).fill(
  ...
  let hosp =
    props.hearts > 2
      ? []
      : [<img src={ambu} alt="" className="list-hosp"></img>];

  let feed =
    props.foodLast > 5
      ? [<img src={eat} alt="" className="list-eat"></img>]
      : [];
